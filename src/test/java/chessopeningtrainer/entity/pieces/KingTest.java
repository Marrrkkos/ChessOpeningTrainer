...
class KingTest {
    ...
    @Test
    void getBasicKingMoves() {
        Field field = board.getBoard()[3][3];   //middle
        field.setPiece(new King(true, true));
        ...
        assertEquals(8, possibleMoves.stream().mapToInt(List::size).sum());     //Castling in specialMoves

        Field field1 = board.getBoard()[0][0];  //corner
        field1.setPiece(new King(true, true));
        List<List<Position>> possibleMoves1 = field1.getPiece().getBasicPieceMoves(new Position(0,0));
        assertEquals(3, possibleMoves1.size());  // 3 directions

        assertEquals(3, possibleMoves1.stream().mapToInt(List::size).sum());     //Castling in specialMoves
    }
